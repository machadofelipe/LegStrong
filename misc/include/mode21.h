...
//            uint8_t     Battery_temperature;       // Battery temperature, [�C], [-40�C - 215�C], [A-40]
...
//            uint16_t    Rear_wheel_speed_avg;       // Rear wheel speed, [km/h],  [0km/h - 65.535km/h], [(256*A+B)/1000]
            uint16_t    Motor_kRPM;                  // Inside hub motor estimate rpm, [rpm],   [0rpm - rpm], [(256*A+B)/]
...
//            uint16_t    Cadence_RPM_max;            // Cyclist cadence, [rpm], [0rpm - 218.45rpm], [(256*A+B)/300]
            ...
            // Calculated variables
            uint16_t    Battery_power;              // Power out of the batteries, [W], [-512W - 1536W], [((256*A+B)/32)-512]
